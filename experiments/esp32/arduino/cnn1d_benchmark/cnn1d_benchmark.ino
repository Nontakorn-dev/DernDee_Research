/*
 * CNN1D synthetic benchmark for ESP32-C3 (no IMU/BLE).
 *
 * Head-to-head reference point for the TCN benchmark (tcn_benchmark.ino):
 * CNN1D is the smallest-parameter FP32 baseline in Table I. Only FP32/INT8
 * are exported here (no pruning grid was run for this architecture).
 *
 * Prerequisites (Arduino IDE):
 *   - Board: ESP32C3 Dev Module
 *   - Library: Chirale_TensorFlowLite (TensorFlow Lite Micro)
 *
 * Before flashing, regenerate headers on host:
 ...
 *
 * Switch active config (FP32 needs a larger tensor arena, e.g. 160 KB):
 *   ... --active-config FP32 --tensor-arena-kb 160
 */
...
// CNN1D's TFLite graph only uses TRANSPOSE/RESHAPE/CONV_2D/MEAN/FULLY_CONNECTED
// (verified via tf.lite.experimental.Analyzer); Pad/Add are registered but unused,
// kept identical to tcn_benchmark.ino so both sketches share one resolver contract.
...
void setup() {
  ...
  Serial.println("# CNN1D synthetic benchmark");
  Serial.print("# config=");
